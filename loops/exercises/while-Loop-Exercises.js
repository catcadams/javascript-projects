...
} console.log(`The shuttle fuel level is ${fuelLevel}`);

//b. Use a second loop to query the user for the number of astronauts (up to a maximum of 7). Validate the entry.
  
  while (numOfAstronauts >= 8 || numOfAstronauts <= 1 || isNaN(numOfAstronauts)) {
    numOfAstronauts = input.question("How many astronauts are flying today? ");
  } console.log(numOfAstronauts);
  
  
//c. Use a final loop to monitor the fuel status and the altitude of the shuttle. Each iteration, decrease the fuel level by 100 units for each astronaut aboard. Also, increase the altitude by 50 kilometers.
while (fuelLevel - 100*numOfAstronauts >= 0) {
  fuelLevel = fuelLevel - 100 * numOfAstronauts;
