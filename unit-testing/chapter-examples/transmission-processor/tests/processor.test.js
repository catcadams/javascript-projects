...
describe("transmission processor", function () {
  ...
  test("returns -1 for rawData if > is in middle of rawData", function () {
  ...
  test("returns -1 for rawData if < is not present", function () {
    let result = processor("9701::487297403495720912");
    expect(result.rawData).toBe(-1);
  });
  test("returns -1 for rawData if there are more than two <", function () {
    let result = processor("9701::<487297403495<720912>");
    expect(result.rawData).toBe(-1);
  });
  test("returns -1 if id cannot be converted to a number", function () {
    let result = processor("id::<487297403495720912>");
